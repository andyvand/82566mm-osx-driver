...
OSDefineMetaClassAndStructors(Intel82566MM, super);

void Intel82566MM::free()
{
	e_debug("void Intel82566MM::free()\n");
...
bool Intel82566MM::init(OSDictionary *properties)
{
	e_debug("bool Intel82566MM::Init(OSDictionary *properties).\n");
...
void Intel82566MM::stop(IOService* provider)
{
	e_debug("Intel82566MM::stop(IOService * provider)\n");
...
bool Intel82566MM::start(IOService * provider)
{
	...
	e1000_get_bus_info_pcie_generic(&adapter.hw);
	...
	/* copy the MAC address out of the NVM */
        if (e1000_read_mac_addr_generic(&adapter.hw))
	...
	*/
	...
	netif->registerService();
		...
		this,
		&Intel82566MM::interruptHandler,
		&Intel82566MM::interruptFilter,
		provider);
	...
	interruptSource->enable();
        ...
        //
        watchdogSource = IOTimerEventSource::timerEventSource(this, &Intel82566MM::timeoutHandler );
...
IOReturn Intel82566MM::enable(IONetworkInterface * netif)
{
	int err;
	int detect_times;
	bool link_ok = false;;
...
IOReturn Intel82566MM::disable(IONetworkInterface * netif)
{
	e_debug("Intel82566MM::disable.\n");
	enabledForNetif = false;
...
UInt32 Intel82566MM::outputPacket(mbuf_t skb, void * param)
{

	//e_debug("Intel82566MM::outputPacket().\n");
...
void Intel82566MM::getPacketBufferConstraints(IOPacketBufferConstraints * constraints) const
{
	e_debug("Intel82566MM::getPacketBufferConstraints.\n");
	constraints->alignStart = kIOPacketBufferAlign2;
        constraints->alignLength = kIOPacketBufferAlign1;
	return;
}

IOOutputQueue * Intel82566MM::createOutputQueue()
{
	e_debug("Intel82566MM::createOutputQueue().\n");
	return IOGatedOutputQueue::withTarget(this, getWorkLoop());
}

const OSString * Intel82566MM::newVendorString() const
{
	e_debug("const OSString * Intel82566MM::newVendorString() const.\n");
	return OSString::withCString("Intel");
}

const OSString * Intel82566MM::newModelString() const
{
	e_debug("const OSString * Intel82566MM::newModelString() const.\n");
	return OSString::withCString("82566MM");
}

IOReturn Intel82566MM::selectMedium(const IONetworkMedium * medium)
{
	e_debug("IOReturn Intel82566MM::selectMedium(const IONetworkMedium * medium).\n");
	bool link;
	SInt32 ret_val;
...
bool Intel82566MM::configureInterface(IONetworkInterface * interface)
{
	e_debug("Intel82566MM::configureInterface.\n");
...
bool Intel82566MM::createWorkLoop()
{
	e_debug("Intel82566MM::createWorkLoop().\n");
	workLoop = IOWorkLoop::workLoop();	
	return (workLoop !=  NULL);
}

IOWorkLoop * Intel82566MM::getWorkLoop() const
{
	e_debug("Intel82566MM::getWorkLoop() const.\n");
	return workLoop;
}
...
IOReturn Intel82566MM::getHardwareAddress(IOEthernetAddress * addr)
{
	e_debug("Intel82566MM::getHardwareAddress(IOEthernetAddress * addr).\n");
	memcpy(addr, adapter.hw.mac.addr, kIOEthernetAddressSize);
	return kIOReturnSuccess;
}
IOReturn Intel82566MM::setHardwareAddress(const IOEthernetAddress * addr)
{
	e_debug("Intel82566MM::setHardwareAddress(const IOEthernetAddress * addr).\n");
	return kIOReturnSuccess;
}
IOReturn Intel82566MM::setPromiscuousMode(bool active)
{
	e_debug("Intel82566MM::setPromiscuousMode(bool active).\n");
	return kIOReturnSuccess;
}
IOReturn Intel82566MM::setMulticastMode(bool active)
{
	e_debug("Intel82566MM::setMulticastMode(bool active).\n");
	
	return kIOReturnSuccess;
}
IOReturn Intel82566MM::setMulticastList(IOEthernetAddress * addrs, UInt32 count)
{
	e_debug("Intel82566MM::setMulticastList(IOEthernetAddress * addrs, UInt32 count).\n");
...
IOReturn Intel82566MM::getChecksumSupport(UInt32 *checksumMask, UInt32 checksumFamily, bool isOutput) 
{
        if( checksumFamily != kChecksumFamilyTCPIP ) {
                IOLog("Intel82566MM: Operating system wants information for unknown checksum family.\n");
...
//-----------------------------------------------------------------------
bool Intel82566MM::addNetworkMedium(UInt32 type, UInt32 bps, UInt32 index)
{
...
void Intel82566MM::interruptOccurred(IOInterruptEventSource * src)
{
	int i;
	struct e1000_hw* hw = &adapter.hw;
...
void Intel82566MM::interruptHandler(OSObject * target, IOInterruptEventSource * src, int count)
{
	Intel82566MM * me = (Intel82566MM *) target;
	me->interruptOccurred(src);
	return;
}

bool Intel82566MM::interruptFilter(OSObject * target, IOFilterInterruptEventSource * src )
{
        Intel82566MM * me = (Intel82566MM *) target;
...
SInt32 Intel82566MM::e1000_read_pcie_cap_reg(struct e1000_hw *hw, UInt32 reg, UInt16 *value)
{
	e_debug("Intel82566MM::e1000_read_pcie_cap_reg(struct e1000_hw *hw, UInt32 reg, UInt16 *value)");
 ...
 **/
SInt32 Intel82566MM::e1000_setup_ring_resources(struct e1000_ring* ring)
{
	void *desc;
	...
	IOLog("ring desc success: size->%d, virtual:%08x, phy:%08x.\n", SIZE_RING_DESC, desc, dma);
 ...
 **/
void Intel82566MM::e1000_configure_tx()
{
	struct e1000_hw *hw = &adapter.hw;
	struct e1000_ring *tx_ring = &adapter.tx_ring;
 ...
 * @adapter: Board private structure
 **/
void Intel82566MM::e1000_setup_rctl()
{
	struct e1000_hw *hw = &adapter.hw;
	UInt32 rctl;
 ...
 **/
void Intel82566MM::e1000_configure_rx()
{
	struct e1000_hw *hw = &adapter.hw;
 ...
 **/
int Intel82566MM::e1000_desc_unused(struct e1000_ring *ring)
{
 ...
 **/
void Intel82566MM::e1000_alloc_rx_buffers()
{
	struct e1000_ring *rx_ring = &adapter.rx_ring;
...
bool Intel82566MM::e1000_clean_rx_irq()
{
        struct e1000_ring *rx_ring = &adapter.rx_ring;
 ...
 **/
bool Intel82566MM::e1000_clean_tx_irq()
{
        struct e1000_ring *tx_ring = &adapter.tx_ring;
...
void Intel82566MM::e1000e_enable_receives()
{
...
bool Intel82566MM::e1000_has_link()
{
        struct e1000_hw *hw = &adapter.hw;
        bool link_active = 0;
        SInt32 ret_val = 0;
 ...
 * @adapter: board private structure
 **/
void Intel82566MM::e1000_clean_tx_ring()
{
  ...
  **/
 void Intel82566MM::e1000_free_tx_resources()
 {
	 struct e1000_ring *ring = &adapter.tx_ring;
 ...
 * @adapter: board private structure
 **/
void Intel82566MM::e1000_clean_rx_ring()
{
...
void Intel82566MM::e1000_free_rx_resources()
{
	struct e1000_ring *ring = &adapter.rx_ring;
...
void Intel82566MM::timeoutOccurred(IOTimerEventSource* src)
{
	...
	//e_debug("timeout link:%d, preLinkStatus:%d.\n", link, preLinkStatus);
...
void Intel82566MM::timeoutHandler(OSObject * target, IOTimerEventSource * src)
{
	//e_debug("void Intel82566MM::timeoutHandler(OSObject * target, IOTimerEventSource * src)\n");
	Intel82566MM* me = (Intel82566MM*) target;
	me->timeoutOccurred(src);
	
}

void Intel82566MM::e1000_print_link_info()
{
        struct e1000_hw *hw = &adapter.hw;
...
bool Intel82566MM::e1000_set_multi(IOEthernetAddress * addrs, UInt32 count)
{
        struct e1000_hw *hw = &adapter.hw;
	...
	unsigned char* mta_list;
		...
		mta_list = (unsigned char*)mta_desc->getBytesNoCopy();
                ...
                hw->mac.ops.update_mc_addr_list(hw, mta_list, i, 1, mac->rar_entry_count);
		mta_desc->release();
        } else {
               /*
		...
		E1000_WRITE_REG(hw, E1000_RCTL, rctl);
                hw->mac.ops.update_mc_addr_list(hw, NULL, 0, 1, mac->rar_entry_count);
        }
	return true;
}

bool Intel82566MM::e1000_tx_csum(mbuf_t skb)
{
/*
        struct e1000_ring *tx_ring = &adapter.tx_ring;
